...
const MemberSinceData = ({ createdAt }) => {
  ...
  if (years > 0) {
    ...
    memberSincePara = `Member since ${months} month${
      months > 1 ? "s" : ""
    } ago`;
  } else if (isNaN(days)) {
...
const Profile = () => {
  ...
  useEffect(() => {
    axios
      .get(`${urlHome}/user?email=${user.email}`, { withCredentials: true })
      .then((res) => {
  ...
  useEffect(() => {
    axiosSecure
      .get(`/activity?email=${user.email}`)
      .then((res) => setActivity(res.data))
      .catch((error) => console.error(error));
    axiosSecure
      .get(`/countUsers?email=${user.email}`)
  ...
  return (
    <section className="bg-gray-100 min-h-screen">
      <div className="w-11/12 mx-auto">
        <Helmet>
          <title>
            Profile | {import.meta.env.VITE_NAME || "ProRecommendation"}
          </title>
        </Helmet>
          ...
          <div className="flex  items-center space-x-4">
            ...
            <div>
              ...
              <div className="text-gray-500">
                <MemberSinceData createdAt={userData?.createdAt} />
              </div>
            </div>
          </div>
          <NavLink
            ...
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <TabPanel>
                  <div className="p-2">
                    <p>
                      Static Bio : I am a programmer. My principal language is
                      C++. I've also done commercial work in Java, C, Perl,
                      Python, JavaScript, and APL. I've also been known to
                      dabble in Lisp, Haskell, assembler (ARM, x86, amd64), and
                      probably a few other languages that haven't left as big a
                      mark.
                    </p>
                    <p className="mt-4">
                      Programmer at{" "}
                      ...
                      . Posts and comments are my opinions and are not sponsored
                      or endorsed by my employer.
                    </p>
                    <p className="mt-4">
                      If my answers helped you, you can{" "}
                      <a href="#" className="text-blue-500">
                        buy me a coffee
                      </a>
                      .
                ...
                <TabPanel>
                  <div className="p-3">
                    {activity.length > 0 ? (
                      activity.map((act) => (
                        <p className="p-2 " key={act._id}>
                          {" "}
                          {++i}.{" "}
                          <span className="uppercase font-semibold">
                            {act.type}
                          </span>{" "}
                          posted on {act.createdAt} about {act.title}{" "}
                        </p>
                      ))
                    ) : (
                      <>No Activity Found!</>
