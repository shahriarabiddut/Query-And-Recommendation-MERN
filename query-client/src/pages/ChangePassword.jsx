...
export default function ChangePassword() {
  ...
  const { passwordReset, showToast, email, setEmail } = useAuth();
  ...
  return (
    <div className="min-h-fit flex justify-center items-center py-10">
      <Helmet>
        <title>
          Change Password | {import.meta.env.VITE_NAME || "ProRecommendation"}
        </title>
      </Helmet>
      <div className="w-10/12 md:w-10/12 lg:w-1/3 mx-auto my-4">
        ...
        <div className="mx-auto">
          {error && error.length > 0 && (
            <div className="bg-red-500 text-center rounded-xl p-3 mt-5 grid gap-1 text-white">
              {error.map((e, i) => (
                <p key={i}>{e}</p>
              ))}
            </div>
          ...
          {success && (
            <div className="bg-green-500 text-center rounded-xl p-3 mt-5 text-white">
              Successfully Change Password Email Sent!
            </div>
          )}
          <form
          ...
          >
            ...
            <div className="form-control my-6">
              <button className="btn bg-blue-700 text-white font-semibold hover:text-blue-800 hover:font-bold hover:bg-white">
                Reset
              </button>
            </div>
            <div className="form-control">
              <label className="label">
                <span className="label-text">
                  New to this website?{" "}
                  <NavLink
                    className="text-blue-600 font-bold hover:text-purple-800"
                    to="/auth/register"
                  >
                    Create an Account
                  </NavLink>{" "}
