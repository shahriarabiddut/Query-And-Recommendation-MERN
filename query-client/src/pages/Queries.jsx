...
const Queries = () => {
    ...
    //
    useEffect(()=>{
        axiosSecure.get(`/queries?page=${currentPage}&size=${itemsPerPage}`).then(res => setQueries(res.data)).catch((error) =>console.error(error));
        setTimeout(() => {
            setLoader(false);
          }, 3000);
    ...
    const handleSubmit = (e)=>{
        ...
        if(samePage){
            ...
            axiosSecure.get(`/querySearch?name=${searchVal}`).then(res => setQueries(res.data)).catch((error) =>console.error(error));
            setTimeout(() => {
                setLoader(false);
              }, 3000);
        }
        ...
        <section >
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 justify-center items-center gap-4'>
                <div className='flex justify-center items-center gap-2'>
                    <p className="font-barlowtext-2xl"> Layout : </p>
                        <button className="p-2 rounded-xl bg-blue-500 text-white flex gap-2 items-center" onClick={() => setColumns(1)}>
                            <TbColumns1  className='text-xl' /> 1 Column
                        </button>
                        <button className="p-2 rounded-xl bg-blue-500 text-white flex gap-2 items-center" onClick={() => setColumns(2)}>
                ...
                <div className="my-4 flex justify-center md:justify-end gap-2">
                    ...
                    <input type="text" className="w-3/4 grow" 
            ...
            { searched != '' &&
                <div className='flex flex-wrap gap-3 md:justify-between items-center'>
                    <h1 className='p-3 bg-slate-100 rounded-sm'>You have Searched <span className='font-bold '>{searched}</span></h1>
                    <Link to={'/queries'} className='p-2 rounded-xl bg-purple-500 text-white flex gap-2 items-center'><FaArrowLeft/> All Queries</Link>
                </div>
            }
            {
                loader ? (
                    ...
                    <>
                        <div className={gridClass}>
                        {
                            filteredQueries.map((query) => (
                                <QueryCard2 key={query._id} query={query} columns={columns}></QueryCard2>
                            ))
                        }
                        </div>
                    </>
