...
const MyRecomendations = () => {
    ...
    //
    useEffect(()=>{
            axiosSecure.get(`/recommendations?email=${user.email}&page=${currentPage}&size=${itemsPerPage}`).then(res => {setRecommendations(res.data);}).catch((error) =>console.error(error));
            setTimeout(() => {
                setLoader(false);
              }, 3000);
    ...
    const handleDelete = (_id)=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosSecure.delete(`/recommendation/${_id}?email=${user.email}`)
                .then(res=>{
                    // console.log(data)
                    if(res.data.deletedCount>0){
                        Swal.fire({
                          ...
                          const remainingRecommendation = recommendations.filter(eq=> eq._id != _id);
                          setRecommendations(remainingRecommendation);
                            axiosSecure.patch(`/recommendation/querycountrem/${queryId}`)
                            .then((res)=>{
    ...
    useEffect( () =>{
        axiosSecure(`/myRecommendationsCount?email=${user.email}`)
                        ...
                        <div className="overflow-x-auto my-10">
                            <table className="table">
                                ...
                                <tr key={recommendation._id}>
                                    ...
                                    <td><button className="inline-flex btn btn-error text-white text-xs" onClick={()=>{handleDelete(recommendation._id);setqueryId(recommendation.queryId)}}> <FaTrash/> Delete</button></td>
                                </tr>))}
                                </tbody>
                            </table>
                            </div>
