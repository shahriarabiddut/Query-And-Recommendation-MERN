...
const HomeQuery = ({category,queriesData,counts}) => {
    const hoverEffect = "bg-gray-200 p-1 rounded-lg hover:bg-blue-700 hover:text-white ";
    ...
    const handleCategoryBtn = (id)=>{
      setLoading(true);
      if(id==0){
        axiosSecure.get(`/queries?page=0&size=6`).then(res=>{setQueries(res.data);setLoading(false);});
      } 
      else{
        axiosSecure.get(`/querySameCategory/${id}`).then(res=>{setQueries(res.data);setLoading(false);});
      }
        ...
        <div className="grid grid-cols-3 md:grid-cols-12 w-11/12 mx-auto relative">
          <div className='col-span-3 flex md:sticky lg:sticky top-20 self-start'>
            <div className='flex flex-row md:flex-col lg:flex-col gap-2 p-6 w-11/12 mx-auto my-3 bg-white rounded-2xl flex-wrap'>
                <button className={ (selectedCategory==0? activeEffect : hoverEffect) + ' flex items-center justify-center gap-2'} onClick={()=>{handleCategoryBtn(0)}}><LiaHomeSolid/> Home</button>
                {category.map( cat =>
                    <button onClick={()=>{handleCategoryBtn(cat.id)}} className={ (selectedCategory==cat.id? activeEffect : hoverEffect) + ' flex items-center justify-center gap-2'} key={cat.id}>{cat.name}</button>
                )}
            </div>
          </div>
          ...
          : queries.length ===0 ?
            <div className='grid justify-center my-10 gap-4'>
              <p className='font-rancho text-5xl'>No Data Found in this Category</p>
          ...
          <div className='col-span-3'><Achievement counts={counts}/></div>
        </div>
    </section>
  )
}
